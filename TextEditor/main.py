...
def find():
    ...
    labelFrame=LabelFrame(root1,text='Bul/Yer Değiştir')
    labelFrame.pack(pady=50)

    findLabel=Label(labelFrame,text='Bul')
    ...
    replaceLabel = Label(labelFrame, text='Yer Değiştir')
    ...
    findButton=Button(labelFrame,text='BUL',command=find_words)
    findButton.grid(row=2,column=0,padx=5,pady=5)

    replaceButton = Button(labelFrame, text='YER DEĞİŞTİR',command=replace_text)
...
def statusBarFunction(event):
    # if textarea.edit_modified():
        ...
        status_bar.config(text=f'Karakterler: {characters} Kelimeler: {words}')
...
def iexit(event=None):
    if textarea.edit_modified():
        result=messagebox.askyesnocancel('Uyarı','Dosyayı kaydetmek istiyor musunuz?')
        if result is True:
            if url!='':
                content=textarea.get(0.0,END)
                file=open(url,'w')
...
filemenu.add_command(label='Yeni',accelerator='Ctrl+N',image=newImage,compound=LEFT,command=new_file)
filemenu.add_command(label='Aç',accelerator='Ctrl+O',image=openImage,compound=LEFT,command=open_file)
filemenu.add_command(label='Kaydet',accelerator='Ctrl+S',image=saveImage,compound=LEFT,command=save_file)
filemenu.add_command(label='Farklı Kaydet',accelerator='Ctrl+Alt+S',image=saveasImage,compound=LEFT,command=saveas_file)
filemenu.add_command(label='Yazdır',accelerator='Ctrl+P',image=printImage,compound=LEFT,command=printout)

filemenu.add_separator()
filemenu.add_command(label='Çıkış',accelerator='Ctrl+Q',image=exitImage,compound=LEFT,command=iexit)

#düzenleme bölümü menüsü
...
status_bar=Label(root,text='Durum Çubuğu')
...
editmenu.add_command(label='Geri Al',accelerator='Ctrl+Z',image=undoImage,compound=LEFT,
                     command=lambda :textarea.event_generate('<Control z>'))
editmenu.add_command(label='Kes',accelerator='Ctrl+X',image=cutImage,compound=LEFT,
                     command=lambda :textarea.event_generate('<Control x>'))
editmenu.add_command(label='Kopyala',accelerator='Ctrl+C',image=copyImage,compound=LEFT,
                     command=lambda :textarea.event_generate('<Control c>'))
editmenu.add_command(label='Yapıştır',accelerator='Ctrl+V',image=pasteImage,compound=LEFT,
                     command=lambda :textarea.event_generate('<Control v>'))

editmenu.add_command(label='Hepsini Seç',accelerator='Ctrl+A',image=selectImage,compound=LEFT,
                     command=lambda: textarea.event_generate('<Control a>'))

editmenu.add_command(label='Temizle',accelerator='Ctrl+Alt+X',image=clearImage,compound=LEFT,
                     command=lambda :textarea.delete(0.0,END))
editmenu.add_command(label='Bul',accelerator='Ctrl+F',image=findImage,compound=LEFT,command=find)
editmenu.add_command(label='Saat/Tarih',accelerator='Ctrl+D',image=datetimeImage,compound=LEFT
                     ,command=date_time)
...
viewmenu.add_checkbutton(label='Araç Çubuğu',variable=show_toolbar,onvalue=True,offvalue=False,image=toolbarImage,
                         compound=LEFT,command=toolbarFunc)
show_toolbar.set(True)
viewmenu.add_checkbutton(label='Durum Çubuğu',variable=show_statusbar,onvalue=True,offvalue=False,image=statusImage,
                         compound=LEFT,command=statusbarFunc)
show_statusbar.set(True)
menubar.add_cascade(label='Görünüm',menu=viewmenu)

#temalar menüsü bölümü
themesmenu=Menu(menubar,tearoff=False)
menubar.add_cascade(label='Temalar',menu=themesmenu)
...
themesmenu.add_radiobutton(label='Varsayılan',image=lightImage,variable=theme_choice,compound=LEFT
                           ,command=lambda :change_theme('white','black'))
themesmenu.add_radiobutton(label='Koyu',image=darkImage,variable=theme_choice,compound=LEFT
                           ,command=lambda :change_theme('gray20','white'))
themesmenu.add_radiobutton(label='Pembe',image=pinkImage,variable=theme_choice,compound=LEFT
